...
sequenceDiagram
    ...
    B->>B: waitForUnlock(channel="lock:release:coupon:issue:42")
    Note over B: EventEmitter + Redis SUB 구독 대기
...
3. **이벤트 기반 대기**: Server B 는 `waitForUnlock` 로직을 통해 `lock:release:*` 채널을 구독하며, Redis 를 반복 폴링하지 않고 이벤트를 기다립니다.
4. **락 해제 + 알림**: Server A 작업 완료 → Redlock `release()` → `PUBLISH` 로 대기자들에게 알립니다. 같은 채널로 내부 EventEmitter 에도 신호가 전달되어 동일 프로세스 내 워커들이 즉시 해제 사실을 알 수 있습니다.
5. **재획득**: Server B 는 이벤트를 수신 후 즉시 `withLock` 을 재시도하여 락을 획득하고 작업을 수행합니다. TTL 내 미완료 시 Redlock 의 automatic extension 이 동작합니다.
...
- **Pub/Sub + EventEmitter**: 실패 시 즉시 재시도하지 않고 이벤트 기반으로 기다려 Redis 부하 완화
