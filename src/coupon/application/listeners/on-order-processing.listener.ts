 ...
 *
 * 수신: order.processing
 * 성공: order.processing.coupon.success 이벤트 발행 (쿠폰 적용 시)
...
export class OnOrderProcessingListener {
  ...
  async handleUseUserCouponByOrder(event: OrderProcessingEvent): Promise<void> {
    ...
    try {
      ...
      this.eventEmitter.emit(
        OrderProcessingCouponSuccessEvent.EVENT_NAME,
        new OrderProcessingCouponSuccessEvent(
          orderId,
          userId,
          coupon.id,
          event.order,
          event.orderItems,
          { id: coupon.id, discountRate: coupon.discountRate },
        ),
      );
    } catch (error) {
      this.logger.error(
